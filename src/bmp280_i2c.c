...
void bme280_init(const bme280_config *config) {
    ...
    gpio_set_function(config->gpio_sck, GPIO_FUNC_I2C);
    gpio_pull_up(config->gpio_sda);
    gpio_pull_up(config->gpio_sck);
...
void bme280_read_raw(int32_t* temp, int32_t* pressure) {
    ...
    i2c_write_blocking(i2c0, ADDR, &reg, 1, true);  // true to keep master control of bus
    i2c_read_blocking(i2c0, ADDR, buf, 6, false);  // false - finished with bus
...
int32_t bme280_convert(int32_t temp, bme280_calib_param* params) {
...
void bme280_get_calib_params(bme280_calib_param* params) {
    ...
    i2c_read_blocking(i2c0, ADDR, buf, NUM_CALIB_PARAMS, false);  // false, we're done reading
